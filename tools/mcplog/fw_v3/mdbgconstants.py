...
def DBG_ATL_PIF_FAIL(args):
    ...
    result = str()
    result += format % (args[0], makeDword(args[1], args[2]), makeDword(args[3], args[4]), makeDword(args[5], args[6]),
                        makeDword(args[7], args[8]), args[9])
...
def DBG_ATL_PHY_DBG_BUF_PART(args):
    numargs = 0
    return _DBG_ATL_PHY_DBG_BUF_PART(args)
    # end of function DBG_ATL_PHY_DBG_BUF_PART
...
def _DBG_ATL_PHY_DBG_BUF_PART(arg):
    print ">>> _DBG_ATL_PHY_DBG_BUF_PART: {}, {}".format(
        ["0x{:04x}".format(a) for a in arg],
        None,
        # phy_dbg_trace.printTrace(None, arg)
    )
    return ">>> _DBG_ATL_PHY_DBG_BUF_PART\n"


msgIds = [
    (0, 'DBG_ATL_PRINTF', _DBG_ID_PRINTF),
    (2, 'DBG_ATL_HELLO', "%s SumOfArguments = %d\n"),
    (3, 'DBG_ATL_GLB_FAULT', "%s\n%sGlobal Fault = %s\n"),
    (10, 'DBG_ATL_PIF_FAIL',
     "[FAIL] Addr: 0x%08X, read: 0x%08X, mask: 0x%08X, expected: 0x%08X, pattern: 0x%08X, errors: %d\n"),
    ...
    (0, 'DBG_ATL_PHY_DBG_BUF_PART', _DBG_ATL_PHY_DBG_BUF_PART),
]
